...
def create_app(test_config: dict[str, Any] | None = None) -> Flask:
    ...
    def inject_user() -> dict[str, Any]:
        return {
            ...
            "asset_version": "20260531-relationship-brief-expanded",
        }
    ...
    def update_account_plan_route(plan_id: int) -> Response:
        ...
        relationship_fields = {
            "company_name": clean_text(payload.get("companyName") if payload.get("companyName") is not None else existing["company_name"]),
            ...
            "company_linkedin_url": clean_text((payload.get("companyLinkedInUrl") if payload.get("companyLinkedInUrl") is not None else payload.get("companyLinkedinUrl")) if (payload.get("companyLinkedInUrl") is not None or payload.get("companyLinkedinUrl") is not None) else existing["company_linkedin_url"]),
            "company_industry": clean_text(payload.get("companyIndustry") if payload.get("companyIndustry") is not None else existing["company_industry"]),
            "company_location": clean_text(payload.get("companyLocation") if payload.get("companyLocation") is not None else existing["company_location"]),
            "company_size": clean_text(payload.get("companySize") if payload.get("companySize") is not None else existing["company_size"]),
            ...
            "key_contact_linkedin_url": clean_text((payload.get("keyContactLinkedInUrl") if payload.get("keyContactLinkedInUrl") is not None else payload.get("keyContactLinkedinUrl")) if (payload.get("keyContactLinkedInUrl") is not None or payload.get("keyContactLinkedinUrl") is not None) else existing["key_contact_linkedin_url"]),
            "key_contact_role": clean_text(payload.get("keyContactRole") if payload.get("keyContactRole") is not None else existing["key_contact_role"]),
            ...
            "cortave_value_proposition": clean_text(payload.get("cortaveValueProposition") if payload.get("cortaveValueProposition") is not None else existing["cortave_value_proposition"]),
            "possible_use_case": clean_text(payload.get("possibleUseCase") if payload.get("possibleUseCase") is not None else existing["possible_use_case"]),
            "suggested_next_step": clean_text(payload.get("suggestedNextStep") if payload.get("suggestedNextStep") is not None else existing["suggested_next_step"]),
            ...
            "detailed_notes": clean_text(payload.get("detailedNotes") if payload.get("detailedNotes") is not None else existing["detailed_notes"]),
        ...
        db.execute(
            ...
            SET account_name = ?, plan_owner = ?, kick_off_date = ?, target_go_live_date = ?,
                ...
                company_name = ?, company_website_url = ?, company_linkedin_url = ?,
                company_industry = ?, company_location = ?, company_size = ?, key_contact_name = ?,
                key_contact_job_title = ?, key_contact_email = ?, key_contact_linkedin_url = ?, key_contact_role = ?,
                relationship_context = ?, cortave_value_proposition = ?, possible_use_case = ?, suggested_next_step = ?,
                relationship_detailed_notes = ?, detailed_notes = ?, meeting_brief_notes = ?,
                updated_at = ?
            WHERE id = ?
            """,
            (
                account_name, plan_owner, kick_off_date, target_go_live_date, territory, current_stage_override, notes,
                relationship_fields["company_name"], relationship_fields["company_website_url"], relationship_fields["company_linkedin_url"],
                relationship_fields["company_industry"], relationship_fields["company_location"], relationship_fields["company_size"], relationship_fields["key_contact_name"],
                relationship_fields["key_contact_job_title"], relationship_fields["key_contact_email"], relationship_fields["key_contact_linkedin_url"], relationship_fields["key_contact_role"],
                relationship_fields["relationship_context"], relationship_fields["cortave_value_proposition"], relationship_fields["possible_use_case"], relationship_fields["suggested_next_step"],
                relationship_fields["relationship_detailed_notes"], relationship_fields["detailed_notes"], relationship_fields["meeting_brief_notes"],
                utc_now(), plan_id,
            ),
        )
        db.commit()
...
def init_db(database_path: str) -> None:
    ...
    db.execute(
        ...
        CREATE TABLE IF NOT EXISTS account_plans (
            ...
            cortave_value_proposition TEXT,
            possible_use_case TEXT,
...
def account_plan_to_dict(row: sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        ...
        "companyLinkedinUrl": row["company_linkedin_url"] or "",
        "companyIndustry": row["company_industry"] or "",
        "companyLocation": row["company_location"] or "",
        ...
        "keyContactLinkedinUrl": row["key_contact_linkedin_url"] or "",
        "keyContactRole": row["key_contact_role"] or "",
        "relationshipContext": row["relationship_context"] or "",
        "cortaveValueProposition": row["cortave_value_proposition"] or "",
        "possibleUseCase": row["possible_use_case"] or "",
        "suggestedNextStep": row["suggested_next_step"] or "",
        "relationshipDetailedNotes": row["relationship_detailed_notes"] or "",
        "detailedNotes": row["detailed_notes"] or "",
